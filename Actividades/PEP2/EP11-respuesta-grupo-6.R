...
datos_anchos <- data.frame(id, casado_m, separado_m, soltero_m)
datos_largos <- datos_anchos %>% pivot_longer(c("casado_m", "separado_m", "soltero_m"),
                                              names_to = "Estado_Civil",
                                              values_to = "Edad")
datos_largos[["Estado_Civil"]] <- factor(datos_largos[["Estado_Civil"]])

g <- ggqqplot(datos_largos, "Edad", facet.by = "Estado_Civil", color = "Estado_Civil")

print(g)

anova <- ezANOVA(datos_largos, dv = Edad, within = Estado_Civil, wid = id, return_aov = TRUE)
...
for(i in 1:R2){
  ...
  bootstrapCasado <- boot(casado_m, statistic = media, R = R2)
  bootstrapCasado <- c(bootstrapCasado$data)
  
  bootstrapSeparado <- boot(separado_m, statistic = media, R = R2)
  bootstrapSeparado <- c(bootstrapSeparado$data)
  
  bootstrapSoltero <- boot(soltero_m, statistic = media, R = R2)
  bootstrapSoltero <- c(bootstrapSoltero$data)
  
  datos_boot <- data.frame(bootstrapCasado, bootstrapSeparado, bootstrapSoltero)
  colnames(datos_boot) <- c("bootstrapCasado", "bootstrapSeparado", "bootstrapSoltero")
  datos_boot <- datos_boot %>% pivot_longer(c("bootstrapCasado", "bootstrapSeparado", "bootstrapSoltero"),
                                            names_to = "Estado_Civil",
                                            values_to = "Edad")
  datos_boot[["Estado_Civil"]] <- factor(datos_boot[["Estado_Civil"]])
  datos_boot[["id"]] <- factor(1:nrow(datos_boot))
  lista <- append(lista, list(datos_boot))
}

listaAnova <- list()

for(i in 1:R2){
  pruebaAnova <- ezANOVA(data = lista[[i]], dv = Edad,
                         between = Estado_Civil, wid = id,
                         return_aov = TRUE)
  listaAnova <- append(listaAnova, list(pruebaAnova$ANOVA$F))
}

# Se calcula el p:
p <- (sum(listaAnova > valor_observado) + 1) / (R2 + 1)
print(p)



obtiene_F <- function(df_ancho){
  df_largo <- df_ancho %>% pivot_longer(c("casado_m", "separado_m", "viudo_m"),
                                        names_to = "Estado_Civil",
                                        values_to = "Edad")
  df_largo[["Estado_Civil"]] <- factor(df_largo[["Estado_Civil"]])
  
  anova <- ezANOVA(df_largo, dv = Edad, within = Estado_Civil, wid = id, return_aov = TRUE)
...
dif_obs_casado_separado <- obtiene_media_difs(datos_anchos, "casado_m", "separado_m")
dif_obs_casado_viudo <- obtiene_media_difs(datos_anchos, "casado_m", "viudo_m")
dif_obs_separado_viudo <- obtiene_media_difs(datos_anchos, "separado_m", "viudo_m")

# Obtiene las distribuciones de las medias de las diferencias permutadas
dist_medias_difs_casado_separado <- sapply(permutaciones, obtiene_media_difs, "casado_m", "separado_m")
dist_medias_difs_casado_viudo <- sapply(permutaciones, obtiene_media_difs, "casado_m", "viudo_m")
dist_medias_difs_separado_viudo <- sapply(permutaciones, obtiene_media_difs, "separado_m", "viudo_m")

num <- sum(abs(dist_medias_difs_casado_separado) > abs(dif_obs_casado_separado)) + 1
den <- R + 1
p_casado_separado <- num / den

num <- sum(abs(dist_medias_difs_casado_viudo) > abs(dif_obs_casado_viudo)) + 1
den <- R + 1
p_casado_viudo <- num / den

num <- sum(abs(dist_medias_difs_separado_viudo) > abs(dif_obs_separado_viudo)) + 1
den <- R + 1
p_separado_viudo <- num / den
...
cat(sprintf("Casado - Separado: %.3f\n", p_casado_separado))
cat(sprintf("Casado - Viudo: %.3f\n", p_casado_viudo))
cat(sprintf("Separado - Viudo: %.3f\n", p_separado_viudo))

cat("\nDiferencias observadas:\n")
cat(sprintf("Casado - Separado: %.3f\n", dif_obs_casado_separado))
cat(sprintf("Casado - Viudo: %.3f\n", dist_medias_difs_casado_viudo))
cat(sprintf("Separado - Viudo: %.3f\n", dist_medias_difs_separado_viudo))
